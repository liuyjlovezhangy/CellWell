...
%     im_expanded = imfilter(im_expanded,h, 'replicate');
    ...
    level = cw.process.adaptiveThresholdFinder(imresize(im,2), 1.5, cseg_tracking_params, []);
...
%     
%     se = strel('disk',2);
%     im_thresh = imclose(im_thresh,se);
    ...
    % fill holes
%     im_thresh = imfill(im_thresh,'holes');
    ...
    objects = regionprops(L,'ConvexHull','Extrema','FilledImage');
...
%             temp_centers_contained_shifted = temp_centers_contained;
            
%             eucl_dist(temp_centers_contained_shifted(:,1),temp_centers_contained_shifted(:,2))
            ...
            j_idcs = linspace(1,size(ti,2),5*sqrt(size(ti,1)^2 + size(ti,2)^2)); % the line will never need to be bigger than the image diagonal
            i_idcs = round(linefun(j_idcs));
            j_idcs = round(j_idcs);
            
            
            for idx = 1:numel(i_idcs)
                if i_idcs(idx) > size(ti,1)
                    continue
                end
...
%             imagesc(ti)
%             
%             plot(temp_centers_contained_shifted(:,1),temp_centers_contained_shifted(:,2),'o-','MarkerSize',30,'LineWidth',5)
...
%     im_thresh_final = bwareaopen(im_thresh_final,50);
    
    L = bwlabeln(im_thresh_final,4);
    objects_final = regionprops(L,'ConvexHull','Centroid');
